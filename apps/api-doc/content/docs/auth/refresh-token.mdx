...
Untuk menyegar semula token akses anda, panggil titik akhir `<SERVER_URL>/webhook/users/refresh-token` menggunakan permintaan `POST`. Anda mesti menyertakan token akses yang sah dalam badan permintaan.
...
`token` yang baharu diperoleh daripada titik akhir penyegaran berfungsi sama seperti token akses yang diterima semasa log masuk. Anda patut menggunakannya dalam pengepala `Authorization` bagi permintaan anda untuk mengakses titik akhir yang dilindungi.
