...
Untuk log masuk, hanya panggil titik akhir `<SERVER_URL>/webhook/users/login` seperti berikut:
...
[Baca lebih lanjut](/docs/auth/refresh-token) untuk mengetahui cara menyegarkan token yang telah tamat tempoh.
...
Anda perlu menyimpan `token` ini dengan selamat kerana ia akan digunakan untuk mengesahkan permintaan seterusnya ke titik akhir yang dilindungi.
...
Untuk mengakses titik akhir yang dilindungi, anda perlu memasukkan `token` dalam header `Authorization` bagi permintaan anda. Token tersebut harus diawali dengan `Bearer `.
