...
Untuk memadam satu lampiran tertentu bagi siaran akhbar, panggil endpoint `<SERVER_URL>/webhook/press-releases/<ID>/attachment/<FILENAME>` menggunakan kaedah `DELETE`. Gantikan `<ID>` dalam URL endpoint dengan pengecam unik siaran akhbar dan `<FILENAME>` dengan nama fail lampiran yang ingin dipadam.
