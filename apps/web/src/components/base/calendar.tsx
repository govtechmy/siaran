...
/**
 * A utility function that will append new class names to the existing class names.
 * @param classNames
 * @param currentClassNames
 * @returns
 */
function appendClassName(
  classNames: Partial<ClassNames>,
  currentClassNames: Partial<ClassNames>,
) {
  const newClassNames: Partial<ClassNames> = {};

  for (const key of Object.keys(currentClassNames)) {
    const typedKey = key as keyof ClassNames;

    newClassNames[typedKey] = cn(
      currentClassNames[typedKey],
      classNames[typedKey],
    );
  }

  return newClassNames satisfies CalendarProps["classNames"];
}
...
export const Calendar = ({
  ...
  return (
    <DayPicker
      ...
      classNames={appendClassName(classNames ?? {}, {
        [UI.Months]: "relative",
        ...
        [UI.DayButton]: cn(
          ...
          "hover:bg-primary hover:text-primary-foreground h-9 w-9 p-0 font-normal aria-selected:opacity-100",
        ),
